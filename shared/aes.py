...
def encypt_message(message: bytes, key: bytes, iv: bytes):
    aes = AES128CTR(key, iv)
    return aes.encrypt(message)


def decrypt_message(message: bytes, key: bytes, iv: bytes):
    aes = AES128CTR(key, iv)
...
class AES128CTR(object):
    def __init__(self, key, iv_bytes=None):
        self._aes = AES(key)

        if iv_bytes is None:
            counter = Counter()
        else:
            counter = Counter(initial_value=int.from_bytes(iv_bytes, "big"))
